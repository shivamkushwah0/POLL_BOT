const {Client , MessageEmbed, Intents, CommandInteractionOptionResolver, MessageReaction} = require('discord.js')
...
const ReactionAdd = (emoji , id ) =>{
    ...
    for(var i = 0; i<polls.length ; i++)
    {
        ...
        {
            if(emoji == "👍")
            polls[i].favour = polls[i].favour + 1;
            else if(emoji == "👎")
            polls[i].oppose = polls[i].oppose + 1;
            break ; 
        }
    }
}
...
client.on('message' ,async (message) => {
	...
	.setThumbnail('https://media-exp1.licdn.com/dms/image/C510BAQEjs-oP2sqmiQ/company-logo_200_200/0/1556714972118?e=2159024400&v=beta&t=VDcc_FH3p9k1DasP5hKWZxYKAsKYvWcHTp2qdzEVfBM')
    ...
    else if(command == "poll-result")
    {
        if(args[0] == undefined)
        {
            message.channel.send("Please give the index of the poll whose result is to be known")
            return ;
        }
        ...
        message.channel.send(`${polls[args[0]-1].favour-2} people are in the favour and ${polls[args[0]-1].oppose-2} oppose the event`)
    }
